...
需要 Git、Python 3、Docker（Apple Silicon 可使用 Colima），以及 Docker Compose 或独立的 docker-compose。`vendor/openmir2` 已随主仓库提交，`vendor/mirserver-data` 使用公开数据子模块；检出时只需初始化数据子模块，所有服务程序从主仓库内的固定源码构建。
...
python3 tools/quest_catalog_audit.py --runtime-mode p0 --json .runtime/reports/quest-catalog-audit.json
