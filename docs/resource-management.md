...
图标按“国服 WIL → 扩展 Crystal Lib”的顺序解析，并在详情中标出实际来源。宽度不超过 4 像素或高度不超过 1 像素的透明解码残片会判为缺图。`item-assets.json` 为同名装备变体、特量药品和损坏的扩展技能书帧声明经过核对的替代图标；技能书沿用 1.76 的通用书本图。当前 1.76 基线全部有图；扩展数据中有 204 个物品和 2 个技能找不到可见的对应素材。角色技能与英雄技能可以共用执行机制 ID，管理器将其显示为机制别名；不同执行机制共用 ID 会列为冲突。
