...
class UiCalibrationTests(unittest.TestCase):
    ...
    def test_calibration_uses_locked_800_by_600_layout(self):
        ...
        self.assertEqual(layout["itemQuickBar"]["x"], 280)
        self.assertEqual(layout["nationalInventoryWindow"]["index"], 3)
        self.assertEqual(layout["nationalCharacterWindow"]["index"], 380)
        helper = (ROOT / "apps/web/src/classic-layout.ts").read_text()
        hud = (ROOT / "apps/web/src/classic-hud.ts").read_text()
        css = (ROOT / "apps/web/src/style.css").read_text()
