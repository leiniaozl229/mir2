            ...
            for (int i = 0; i < GuildList.Count; i++)
            {
                ...
                for (int j = guild.GuildWarList.Count - 1; j >= 0; j--)
                {
                    ...
                    if ((HUtil32.GetTickCount() - warGuild.WarTick) > warGuild.WarTime) //行会战时间到
                    {
                        ...
                        boChanged = true;
                        if (warGuild.Guild != null && warGuild.Guild != guild)
                        {
                            for (int k = warGuild.Guild.GuildWarList.Count - 1; k >= 0; k--)
                            {
                                if (warGuild.Guild.GuildWarList[k].Guild == guild)
                                {
                                    warGuild.Guild.GuildWarList.RemoveAt(k);
                                }
                            }
                            warGuild.Guild.EndGuildWar(guild);
                            warGuild.Guild.UpdateGuildFile();
                        }
                    }
                }
                if (boChanged)
