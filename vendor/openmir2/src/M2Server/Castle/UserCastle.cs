...
namespace M2Server.Castle
{
    public class UserCastle : IUserCastle
    {
        ...
        public void Initialize()
        {
            ...
            if (SystemShare.MapMgr.GetMapOfServerIndex(MapName) == M2Share.ServerIndex)
            {
                ...
                if (CastleEnvir != null)
                {
                    ...
                    {
                        ObjUnit = Archers[i];
                        if (ObjUnit.nHP <= 0 || (ObjUnit.nX == 0 && ObjUnit.nY == 0))
                        {
                            continue;
                        }
        ...
        public void StopWallconquestWar()
        {
            ...
            UnderWar = false;
            AttackGuildList.Clear();
            IList<IPlayerActor> playerList = new List<IPlayerActor>();
            SystemShare.WorldEngine.GetMapRageHuman(CastleEnvir, HomeX, HomeY, 100, ref playerList);
            for (int i = 0; i < playerList.Count; i++)
            {
                IPlayerActor player = playerList[i];
                if (player is PlayObject playObject)
                {
                    playObject.ChangePkStatus(false);
                }
                if (player.MyGuild != MasterGuild)
                {
                    player.MapRandomMove(player.HomeMap, 0);
                }
            }
        ...
        {
            if (PlayObject is PlayObject player)
            {
                return WithDrawalGolds(player, nGold);
            }
            if (nGold <= 0)
            {
                return -4;
        ...
        {
            if (Guild == null || MasterGuild == Guild || InAttackerList(Guild))
            {
                return false;
            }
            AttackerInfo attackerInfo = new AttackerInfo
            {
                AttackDate = M2Share.AddDateTimeOfDay(DateTime.Now, SystemShare.Config.StartCastleWarDays),
                sGuildName = Guild.GuildName,
                Guild = Guild
            };
            AttackWarList.Add(attackerInfo);
            SaveAttackSabukWall();
        ...
        {
            if (guild == null || !UnderWar ||
                (HUtil32.GetTickCount() - StartCastleWarTick) <= SystemShare.Config.GetCastleTime)
            {
                return false;
            }
            IList<IPlayerActor> playerList = new List<IPlayerActor>();
            SystemShare.WorldEngine.GetMapRageHuman(PalaceEnvir, 0, 0, 1000, ref playerList);
            foreach (IPlayerActor player in playerList)
            {
                if (!player.Death && player.MyGuild != guild)
                {
                    return false;
                }
            }
        ...
        {
            return targetObject != null && IsMasterGuild(targetObject.MyGuild) || CheckInPalace(nX, nY);
        }
        ...
        public int ReceiptGolds(IPlayerActor PlayObject, int nGold)
        {
            if (PlayObject is PlayObject player)
            {
                return ReceiptGolds(player, nGold);
            }
            if (nGold <= 0)
            {
                return -4;
        ...
        {
            return AddAttackerInfo((IGuild)Guild);
        }
        ...
        private bool InAttackerList(IGuild Guild)
        {
            ...
            for (int i = 0; i < AttackWarList.Count; i++)
            {
                if (AttackWarList[i].Guild == Guild ||
                    string.Equals(AttackWarList[i].sGuildName, Guild.GuildName, StringComparison.OrdinalIgnoreCase))
                {
                    result = true;
                    break;
                }
        ...
        public bool IsAttackAllyGuild(IGuild Guild)
        {
            ...
            {
                IGuild attackGuild = AttackGuildList[i];
                if (attackGuild != null && attackGuild != MasterGuild && attackGuild.IsAllyGuild(Guild))
                {
                    return true;
                }
            }
        ...
        public bool IsAttackGuild(IGuild Guild)
        {
            ...
            return AttackGuildList.Any(attackGuild => attackGuild == Guild);
        }

        public bool IsDefenseAllyGuild(IGuild guild)
        {
            return UnderWar && MasterGuild != null && guild != null && MasterGuild.IsAllyGuild(guild);
        }

        public bool IsDefenseGuild(IGuild guild)
        {
            return UnderWar && MasterGuild != null && MasterGuild == guild;
        }
    }
}
