...
namespace M2Server.Player
{
    public partial class PlayObject
    {
        ...
        private bool ClientGetButchItem(int charId, int nX, int nY, byte btDir, ref int dwDelayTime)
        {
            ...
            ProtocolTrace.Write($"butch found={baseObject != null} player={CurrX},{CurrY} request={nX},{nY} dead={baseObject?.Death} animal={baseObject?.Animal} skeleton={baseObject?.Skeleton} items={baseObject?.ItemList?.Count} leather={baseObject?.BodyLeathery}");
            if (!SystemShare.Config.CloseSpeedHackCheck)
            {
                int dwCheckTime = HUtil32.GetTickCount() - TurnTick;
        ...
        private void ClientOpenGuildDlg()
        {
            if (MyGuild != null)
            {
                ...
                for (int i = 0; i < MyGuild.GuildWarList.Count; i++)
                {
                    ...
                    WarGuild warGuild = MyGuild.GuildWarList[i];
                    if (warGuild.Guild == null) continue;
                    long remainingMs = warGuild.WarTime - (HUtil32.GetTickCount() - warGuild.WarTick);
                    if (remainingMs > 0)
                    {
                        sSendStr = sSendStr + "+" + warGuild.Guild.GuildName + ' ' + remainingMs + '\r';
                    }
                }
                ...
                for (int i = 0; i < MyGuild.GuildAllList.Count; i++)
                {
                    ...
                    if (MyGuild.GuildAllList[i] != null)
                    {
                        sSendStr = sSendStr + "+" + MyGuild.GuildAllList[i].GuildName + '\r';
                    }
                }
