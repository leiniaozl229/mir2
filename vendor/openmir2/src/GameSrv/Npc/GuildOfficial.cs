    ...
    public class GuildOfficial : NormNpc
    {
        ...
        protected override void GetVariableText(IPlayerActor PlayObject, string sVariable, ref string sMsg)
        {
            ...
            if (sVariable == "$REQUESTCASTLELIST")
            {
                ...
                {
                    string castleName = string.IsNullOrWhiteSpace(List[i]) ? $"城堡{i + 1}" : List[i];
                    sText += Format("<{0}/@requestcastlewarnow{1}> {0}", castleName, i);
                }
                sText = sText + "\\ \\";
        ...
        public override void UserSelect(IPlayerActor PlayObject, string sData)
        {
            ...
            ProtocolTrace.Write($"guild npc select player={PlayObject.ChrName} data={sData}");
            try
            {
                if (!string.IsNullOrEmpty(sData) && sData.StartsWith("@"))
                {
                    ...
                    ProtocolTrace.Write($"guild npc label player={PlayObject.ChrName} label={sLabel} canJump={boCanJmp} payload={sMsg}");
        ...
        {
            string guildName = PlayObject.MyGuild?.GuildName ?? "<none>";
            ProtocolTrace.Write($"guild create begin player={PlayObject.ChrName} name={sGuildName} gold={PlayObject.Gold} guild={guildName}");
            int result = 0;
            ...
            guildName = PlayObject.MyGuild?.GuildName ?? "<none>";
            ProtocolTrace.Write($"guild create end player={PlayObject.ChrName} name={sGuildName} result={result} guild={guildName}");
            return result;
        }
        ...
        {
            IGuild targetGuild = SystemShare.GuildMgr.FindGuild(sGuildName);
            string sourceGuildName = PlayObject.MyGuild?.GuildName ?? "<none>";
            ProtocolTrace.Write($"guild war begin player={PlayObject.ChrName} source={sourceGuildName} target={sGuildName} found={(targetGuild != null)} master={PlayObject.IsGuildMaster()} server={M2Share.ServerIndex} gold={PlayObject.Gold} fee={SystemShare.Config.GuildWarPrice}");
            if (targetGuild != null)
            {
                if (PlayObject.Gold >= SystemShare.Config.GuildWarPrice)
                {
                    ...
                    PlayObject.GoldChanged();
                    PlayObject.ReQuestGuildWar(targetGuild.GuildName);
                    ProtocolTrace.Write($"guild war end player={PlayObject.ChrName} sourceWars={PlayObject.MyGuild?.GuildWarList.Count ?? -1} targetWars={targetGuild.GuildWarList.Count}");
                }
                else
                {
                    ProtocolTrace.Write($"guild war rejected player={PlayObject.ChrName} reason=gold");
                    PlayObject.SysMsg("你没有足够的金币!!!", MsgColor.Red, MsgType.Hint);
                }
            }
            else
            {
                ProtocolTrace.Write($"guild war rejected player={PlayObject.ChrName} reason=missing-target");
        ...
        {
            // The bundled king script uses @requestcastlewarnow for its only
            // castle. Treat the omitted suffix as castle zero while retaining
            // rejection for malformed or out-of-range explicit indices.
            int nIndex = string.IsNullOrWhiteSpace(sIndex) ? 0 : HUtil32.StrToInt(sIndex, -1);
            if (nIndex < 0)
            {
                PlayObject.SysMsg("城堡不存在!!!", MsgColor.Red, MsgType.Hint);
                return;
            }
