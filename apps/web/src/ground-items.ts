...
export class GroundItems {
 ...
 add(item:GroundItem){
  ...
  const visual=new Container();visual.position.set(item.x*48,item.y*32);visual.zIndex=item.y*700+item.x+.25;visual.eventMode='static';visual.cursor='pointer';
  const sprite=new Sprite(Texture.EMPTY),label=new Text({text:item.name,style:{fontFamily:'sans-serif',fontSize:12,fill:0xffe085,stroke:{color:0x000000,width:3}}});
