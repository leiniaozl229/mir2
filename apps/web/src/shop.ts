...
export type ShopGoods={name:string;subMenu:number;price:number;stock:number};
...
export class ShopView {
 ...
 private render(){
  ...
  const heading=document.createElement('div');heading.className='shop-heading';const title=document.createElement('strong');title.textContent=this.mode==='buy'?'商店':'出售物品';const close=document.createElement('button');close.type='button';close.textContent='关闭';close.onclick=()=>this.clear();heading.append(title,close);this.element.append(heading);
