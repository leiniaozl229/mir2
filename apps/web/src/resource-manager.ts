...
function renderOverview(){
 ...
 detail.innerHTML=`<span class="eyebrow">CATALOG HEALTH</span><h2>目录诊断</h2><div class="diagnostic">${diag.itemsMissingIcons} 个物品缺少国服背包图标；当前覆盖 ${catalog.summary.itemIcons}/${catalog.summary.items}。</div><div class="diagnostic">${diag.skillsMissingIcons} 个技能缺少图标；${catalog.summary.skills-catalog.skills.filter(value=>value.rulePinned).length} 个技能仍未接入锁定规则。</div><div class="diagnostic">${diag.monstersMissingVisuals} 个怪物缺少已校准动画映射；当前覆盖 ${catalog.summary.monsterVisuals}/${catalog.summary.monsters}。</div><div class="diagnostic">${diag.orphanSpawnMaps.length} 个刷新地图未进入 570 张基线目录：${escape(diag.orphanSpawnMaps.join('、')||'无')}。</div><div class="diagnostic">${diag.unknownSpawnMonsters.length} 个刷新名称未在怪物表定义；掉落表引用 ${diag.unknownDropItems.length} 个未定义物品。</div><div class="diagnostic">重复 MagicId：${escape(diag.duplicateMagicIds.join('、')||'无')}。</div>`;
}
